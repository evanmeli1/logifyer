...
export const initDatabase = () => {
  ...
  try {
    db.withTransactionSync(() => {
      ...
      if (!incidentColNames.includes('category_name')) {
        db.execSync(`ALTER TABLE incidents ADD COLUMN category_name TEXT;`);
      }
      if (!incidentColNames.includes('category_emoji')) {
        db.execSync(`ALTER TABLE incidents ADD COLUMN category_emoji TEXT;`);
      }
...
export const seedCategories = () => {
  ...
  const defaults = [
    ...
    { name: 'Supported you', emoji: '❤️', points: 5, positive: 1 },
  ];
    ...
    db.withTransactionSync(() => {
      ...
      defaults.forEach(cat => {
        ...
        if (!exists || exists.count === 0) {
          ...
          db.runSync(
            'UPDATE categories SET emoji = ? WHERE name = ? AND is_custom = 0',
            [cat.emoji, cat.name]
          );
        }
      });
...
export const getAllPeople = (): Person[] => {
  ...
  try {
    return db.getAllSync<Person>('SELECT * FROM people WHERE archived = 0 ORDER BY created_at DESC');
  } catch (error) {
...
export const archivePerson = (personId: number): boolean => {
  const db = getDb();
  ...
  try {
    db.runSync('UPDATE people SET archived = 1 WHERE id = ?', [personId]);
    console.log(`✅ Person ${personId} archived`);
    return true;
  } catch (error) {
...
export const getArchivedPeople = (): Person[] => {
  const db = getDb();

  try {
    return db.getAllSync<Person>('SELECT * FROM people WHERE archived = 1 ORDER BY created_at DESC');
  } catch (error) {
...
export const restorePerson = (personId: number): boolean => {
  const db = getDb();
  ...
  try {
    db.runSync('UPDATE people SET archived = 0 WHERE id = ?', [personId]);
    console.log(`✅ Person ${personId} restored`);
    return true;
  } catch (error) {
...
export const getAllCategories = () => {
  ...
  try {
    return db.getAllSync('SELECT * FROM categories ORDER BY is_custom ASC, id ASC');
  } catch (error) {
...
export const getPersonScore = (personId: number): number => {
  ...
  if (!personId || personId <= 0) return 100;

  try {
    const incidents = db.getAllSync<{ points: number }>(
      'SELECT points FROM incidents WHERE person_id = ?',
      [personId]
    );
...
export const getIncidentsByPerson = (personId: number) => {
  ...
  try {
    return db.getAllSync(`
      SELECT i.*,
        COALESCE(i.category_name, c.name, 'Deleted category') as category_name,
        COALESCE(i.category_emoji, c.emoji, '🗑') as category_emoji
      FROM incidents i
...
export const logIncident = (
  ...
  try {
    ...
    const cat = db.getFirstSync<{ name: string; emoji: string }>(
      'SELECT name, emoji FROM categories WHERE id = ?',
      [categoryId]
    );

    db.runSync(
      'INSERT INTO incidents (person_id, category_id, points, is_major, note, feeling_key, timestamp, category_name, category_emoji) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        personId,
        categoryId,
        finalPoints,
    ...
    db.withTransactionSync(() => {
      ...
      defaults.forEach(item => {
        ...
        if (!exists || exists.count === 0) {
          ...
          // Migrate any existing Apple emoji to clean Unicode symbol
          db.runSync(
            'UPDATE relationship_types SET emoji = ? WHERE type = ? AND is_custom = 0',
